...
console.log("Hello from parquetS3");
...
class ParquetReader {
   ...
   * requested. This function returns a new parquet reader or throws an Error.
   */
  static async openS3(client, params, options) {
    let envelopeReader;
    console.error("Where is the error");
    // sanity check (avoid attempts at referencing properties of null)
    //
    if (! client){
      throw Error(
        "Missing S3 client object argument. Pass an instantiated AWS.S3 object "
        + "(v2) or S3Client object (v3) from the AWS SDK."
      );
    }
    // sanity check (avoid attempts at destructuring null)
    try {
      let {Key, Bucket} = params;
      if (! Key || ! Bucket) throw ""; // also checking properties
    }catch (e){
      throw Error(
    ...
    try {
      if ('function' === typeof client.getObject){
        console.log("v2 client detected");
      // S3 client v2
        envelopeReader = ParquetEnvelopeReader.openS3(client, params, options);
      }
      else{
        console.log("v3 client detected");
      // S3 client v3
        let {S3Client:c, HeadObjectCommand:h, GetObjectCommand:g} = client;
        envelopeReader = ParquetEnvelopeReader.openS3v3(c, h, g, params, options);
      }
    }
    catch (e){
      return Promise.reject("Error accessing S3 bucket: " + e.message);
    }
    return this.openEnvelopeReader(envelopeReader, options);
  }
...
class ParquetEnvelopeReader {
  ...
  static async openS3v3(client, headCommand, getCommand, params, options) {
    return new Promise (async (resolve, reject) => {

      let fileStat = async () => {
        try {
         let headObjectCommand = await client.send(
           new headCommand (params)
          );
      ...
      let readFn = async (offset, length, file) => {
        ...
        try {
          let getObjectCommand = await client.send(
            new getCommand({Range: range, ...params})
          );
